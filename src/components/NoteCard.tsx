...
export function NoteCard({ note, isExpanded, onToggle }: NoteCardProps) {
  const title = deriveTitle(note)
  const date = formatDate(note.created_at)
  const duration = formatDuration(note.duration_seconds)

  return (
    <article className="group rounded-none border border-retro-cyan/20 bg-retro-dark transition hover:border-retro-cyan/50 hover:shadow-pixel">
      <button
        type="button"
        onClick={onToggle}
